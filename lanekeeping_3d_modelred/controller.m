...
function Outputs(block)
  ...
  % read input - vehicle state
  x = block.InputPort(1).Data;

  N = 20;
  [deltaf, cost] = dyn.solve_mpc(x, diag(repmat([1 0 0], 1, N)), zeros(N*3,1), 4*eye(N), zeros(N,1), repmat(C,1,N), opts);

  block.OutputPort(1).Data = deltaf(1);
