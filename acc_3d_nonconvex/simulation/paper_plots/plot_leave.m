...
% Carsim data interval
res = 5;

% filename 
filename = 'sim_case3_leave.tikz';

%%%%%%%%%%%% PLOT SIMULINK %%%%%%%%%%%%%%%%%%

load sim_leave_simulink
...
slink = plot(x_out.time, x_out.signals.values(:,1), 'b', 'linewidth', lw)
if plot_after_5s
	lead = plot(x_out.time(t5:end), x_out.signals.values(t5:end,3), 'r', 'linewidth', lw)
else
	lead = plot(x_out.time(1:t5-1), x_out.signals.values(1:t5-1,3), 'r', 'linewidth', lw)
end
...
load sim_leave_carsim
t5 = find(x_out.time >= 5, 1, 'first');
t20 = find(x_out.time >= 20, 1, 'first')-1;
t20u = find(u_out.time >= 20, 1, 'first')-1;

subplot(411)
carsim = plot(x_out.time(1:res:t20), x_out.signals.values(1:res:t20,1), 'k--', 'linewidth', lw)

legend([slink carsim lead], 'ACC-Simulink', 'ACC-Carsim', 'Lead', 'Location', 'NorthOutSide')

subplot(412)
if plot_after_5s
	plot(x_out.time(t5:res:t20), x_out.signals.values(t5:res:t20,2), 'k--', 'linewidth', lw)
else
	plot(x_out.time(1:res:(t5-1)), x_out.signals.values(1:res:(t5-1),2), 'k--', 'linewidth', lw)
end

subplot(413)
plot(u_out.time(1:res:t20u), u_out.signals.values(1:res:t20u)/(9.82*con.mass), 'k--', 'linewidth', lw)

subplot(414)
plot(x_out.time(1:res:t20), max(0, min(3, x_out.signals.values(1:res:t20,2)./x_out.signals.values(1:res:t20,1))), 'k--', 'linewidth', lw)
