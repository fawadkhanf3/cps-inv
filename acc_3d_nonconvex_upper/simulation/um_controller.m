...
function InitializeConditions(block)
  ...
  assignin('base','Cinv_upper',Cinv_upper);
  assignin('base','Cinv_lower',Cinv_lower);
...
function Outputs(block)
  ...
  [current_poly_upper] = find_cell(Cinv_upper, x0);
  [current_poly_lower] = find_cell(Cinv_lower, x0);

  if current_poly_upper == -1;
    disp([num2str(block.currentTime), ': Upper cell not found for ', mat2str(x0), ', keeping constant'])
    % block.OutputPort(1).Data = con.umin;
    return;
  end
  if current_poly_lower == -1;
    disp([num2str(block.currentTime), ': Lower cell not found for ', mat2str(x0), ', keeping constant'])
    % block.OutputPort(1).Data = con.umin;
    return;
  end
  ...
  umin = 0;
  cmin = Inf;
  for i=current_poly_upper:-1:current_poly_upper-1
    for j=current_poly_lower:-1:current_poly_lower-1
      ipoly = intersect1(Cinv_upper(i), Cinv_lower(j));
      [u, c, f] = region_dyn.solve_mpc(x0, Rx, rx, Ru, ru, ipoly);
      if f && c < cmin
        umin = u;
        cmin = c;
      end
...
function [ind1] = find_cell(cmat, x0)
  ind1 = -1;
  for i=1:length(cmat)
      if contains1(cmat(i),x0)
        ind1 = i; 
        return;
      end
  end
...
function  [Rx,rx,Ru,ru] = mpcweights(v,d,vl,udes,N,con)
  ...
  tau_des = (con.tau_des_min + con.tau_des_max)/2;
  v_goal = min((con.v_des_max+con.v_des_min)/2, vl);
  h_goal = max(3, tau_des*v);
