function [t_vec, x_vec, u_vec] = simulate_3d(pwd, cvec)
	...
	for i=1:length(t_vec)	
		...
		next_poly_nr = get_poly(cvec, x);
		next_poly = cvec(next_poly_nr);
		...
		u = this_dyn.solve_mpc(x, Rx, rx, Ru, ru, next_poly);
	...
	[ax,h1,h2] = plotyy(t_vec, x_vec([1 3],:), t_vec,  min(3, x_vec(2,:)./x_vec(1,:)));
	...
	plot(cvec(1),'alpha',0.1, 'color', 'blue');
...
function a = get_disturbance(t, x, dyn)
	...
	a = (t<30)*dmax + (t>30)*(t<55)*dmin + (t>75)*(t<120)*dmax*0.3;
	return;
end